...
trait SharedBehaviors {
  ...
  private[raft] lazy val initialConfigurationBehavior: StateFunction = {
    ...
    // todo For now, non-matching msg is evaluated as a state message sent by the dispatcher
    case _ =>
      //println("SENDER: " + sender().path) // sends himself unhandled msg
      if(sender() != self) {
        //  println("I received sth, probably state ")
        sender() ! getInternalState
      }
      stay()
  }
