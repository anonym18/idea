...
case class ReplicatedLog[Command](
  ...
  override def toString = entries.toString() //B

  def prettyPrintLogs() = entries.map(p => p.command.toString).mkString("\n") //B
}
...
case class Entry[T](
  ...
  override def toString = client match {
    case Some(c) => "Entry(" + command + "," + term + "," + index + "," + c.path.name + ")"
    case None => "Entry(" + command + "," + term + "," + index + ",NO_CLIENT)"
  }
}
