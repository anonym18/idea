...
private[raft] trait Leader {
  ...
  val leaderBehavior: StateFunction = {
    ...
    case Event(ClientMessage(client, cmd: Command), m: LeaderMeta) =>
      println("Handling CLIENT MSG: " + client + "  " + cmd + " written to matchIndex ")

      //matchIndex: index of the highest log entry known to be replicated on server
      log.info("\nAppending command: [{}] from {} to replicated log...", cmd, client)

      val entry = Entry(cmd, m.currentTerm, replicatedLog.nextIndex, Some(client))

      log.debug("adding to log: {}", entry)
      replicatedLog += entry  // after it is selected as a leader, gets replicated log, normally should be sent to the followers

      // entry.index - nextIndex =
      matchIndex.put(m.clusterSelf, entry.index) //B entry index is the replicatedLog.nextIndex

      log.debug("log status = {}", replicatedLog)

      //println("PUT: "+ m.clusterSelf + "  " + entry.index)
      //println("Hence, the last index known to be replicated is: " + entry.index)
      //println("Match index: " + matchIndex.toString)
  ...
  def initializeLeaderState(members: Set[ActorRef]) {
    ...
    //println(Console.RED, "Preparing nextIndex and matchIndex table for followers, init all to: replicatedLog.lastIndex = {}", replicatedLog.lastIndex, Console.BLACK)

    nextIndex = LogIndexMap.initialize(members, replicatedLog.lastIndex) // in init, set to 0, when elected as leader, set to 1 (after processing two client msgs)
  ...
  def startHeartbeat(m: LeaderMeta) {
    ...
    //setTimer(HeartbeatTimerName, SendHeartbeat, heartbeatInterval, repeat = true) //B
    MockTime.time.scheduler.schedule(FiniteDuration(0, TimeUnit.MILLISECONDS), heartbeatInterval, new Runnable() {
  ...
  def replicateLog(m: LeaderMeta) {
    //println(Console.RED, "Replicating log..", Console.BLACK)
    //println("Replicated log: " + replicatedLog.toString, " Size: " + replicatedLog.nextIndex)
...
//      log.info("sending: {} to {}", AppendEntries(m.currentTerm, replicatedLog, fromIndex = nextIndex.valueFor(member), leaderCommitId = replicatedLog.committedIndex), member)
      ...
      //println("Sending to " + member.toString + " Size: " + replicatedLog.nextIndex)
      //println("Term: " + m.currentTerm)
      //println("fromIndex: " + nextIndex.valueFor(member))

      //println(Console.RED,"Match index: " + matchIndex.valueFor(member))
      //println(Console.RED,"Next index: " + nextIndex.valueFor(member), Console.BLACK)
      //println(Console.RED,"Committed index: " + replicatedLog.committedIndex, Console.BLACK)
