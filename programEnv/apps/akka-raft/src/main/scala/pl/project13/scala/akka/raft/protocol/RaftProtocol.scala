...
trait RaftProtocol extends Serializable {
  ...
  case class ClientMessage[T](client: ActorRef, cmd: T) extends RaftMessage {
    override def toString: String = "ClientMessage(" + client.path.name + "," + cmd + ")"
  }
  ...
  case class RequestVote(
    ...
    override def toString: String = "RequestVote(" + term + "," + candidateId.path.name + "," + lastLogTerm + "," + lastLogIndex + ")"
  }
  ...
  case class AppendEntries[T](
    ...
    //override def toString = s"""AppendEntries(term:$term,prevLog:($prevLogTerm,$prevLogIndex),""" + "\n" + "entries:" + entries
    //override def toString = "AppendEntries(term:" + term + ",prevLog:(" + prevLogTerm + "," + prevLogIndex + ")\n" + s"""entries:$entries)"""
    override def toString = "AppendEntries(term:" + term + ",prevLog:(" + prevLogTerm + "," + prevLogIndex + "),\n" + "entries:" + entries + ")"
  }
