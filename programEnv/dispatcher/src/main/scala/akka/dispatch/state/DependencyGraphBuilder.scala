...
class DependencyGraphBuilder {
  ...
  private val MESSAGE_SEND = "MESSAGE_SEND"
  private val CREATOR = "CREATOR"
  private val EXECUTED_ON_SENDER = "EXECUTED_ON_SENDER"
  private val EXECUTED_ON_CREATOR = "EXECUTED_ON_CREATOR"
  private val SENDER_RECEIVER = "SENDER_RECEIVER"
  ...
  private def addExecutedOnSenderCausality(cause: Message, dependent: Message): Unit = {
    val msgsExecutedOnSender: Set[MessageId] = actorProcessedMap.getOrElse(cause.receiver, Set()).map(_.id)
    msgsExecutedOnSender.foreach(msgId => executedOnSenderCausalityMap += (dependent.id -> Set((EXECUTED_ON_SENDER, msgId))))
  }
  ...
  private def addExecutedOnCreatorCausality(cause: Message, dependent: Message): Unit = actorCreatedByMap.get(dependent.receiver) match {
    case Some(actorThatCreatedReceiver) =>
      val msgsExecutedOnCreator = actorProcessedMap.getOrElse(actorThatCreatedReceiver, Set()).map(_.id)
      msgsExecutedOnCreator.foreach(msgId => executedOnCreatorCausalityMap += (dependent.id -> Set((EXECUTED_ON_CREATOR, msgId))))
    case None => // do nth
  }
  ...
  /**
    ...
    * @return predecessors of the sent messages
    */
