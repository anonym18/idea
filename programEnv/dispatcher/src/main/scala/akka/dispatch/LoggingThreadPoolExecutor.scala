...
class LoggingThreadPoolExecutor(corePoolSize: Int, maxPoolSize: Int, threadTimeout: Duration,
...
final case class LoggingThreadPoolConfig(allowCorePoolTimeout: Boolean = ThreadPoolConfig.defaultAllowCoreThreadTimeout,
  ...
  class ThreadPoolExecutorServiceFactory(val threadFactory: ThreadFactory) extends ExecutorServiceFactory {
    def createExecutorService: ExecutorService = {
      val service = new LoggingThreadPoolExecutor(
        corePoolSize,
        maxPoolSize,
        threadTimeout,
        threadFactory,
